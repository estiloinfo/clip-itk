...
static int 
Base64Encode(char* message, char** buffer) 
{
	BIO *bio, *b64;
	FILE* stream;
...
static int 
Base64Decode(char* b64message, char** buffer) 
{ 
	BIO *bio, *b64;
...
int
clip_BASE64DECODE(ClipMachine *mp)
{
	int len = 0;
	...
	char* base64DecodeOutput;
        ...
        {
                _clip_retc(mp, "");
                return _clip_trap_err(mp, EG_ARG, 0, 0, __FILE__, __LINE__, "BASE64DECODE");
        }

        Base64Decode(str, &base64DecodeOutput);

        _clip_retc(mp, base64DecodeOutput );
...
clip_BASE64ENCODE(ClipMachine *mp)
{
	...
	char* base64EncodeOutput;
        ...
        {
                _clip_retc(mp, "");
                return _clip_trap_err(mp, EG_ARG, 0, 0, __FILE__, __LINE__, "BASE64ENCODE");
        }

        Base64Encode(str, &base64EncodeOutput);

        _clip_retc(mp, base64EncodeOutput );
