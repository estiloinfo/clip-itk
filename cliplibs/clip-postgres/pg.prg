...
FUNCTION PG2CLIP(rowset,fieldno)
	...
	SWITCH (type)
		...
		CASE PGT_OID8
			...
			ENDIF
		CASE PGT_VARCHAR,PGT_TEXT,PGT_BYTEA,PGT_BPCHAR,PGT_CHAR
			RETURN data
	...
	SWITCH (type)
		...
		CASE PGT_BOOL
			...
			ELSE
				RETURN IF(value,"t","f")
			ENDIF
		CASE PGT_VARCHAR,PGT_TEXT,PGT_BPCHAR,PGT_BYTEA,PGT_FILENAME,PGT_NAME,PGT_CHAR
			IF totext
				RETURN ADDAPOST(value)
			ENDIF
			RETURN value
		CASE PGT_BOX
			IF !totext .AND. PG_ISBINARY(rowset)
